...
setwd("~/Workspace/06_mdd.liver/paper/clustering-total/cellchat")
setwd("./epi.c8-immune")

# load mdd.filter1
load("~/Workspace/06_mdd.liver/paper/clustering-total/mdd.liver.filter1/mdd.filter1.RData")
scRNA <- subset_cells 
...
# load epi.filter1
load("~/Workspace/06_mdd.liver/paper/clustering-epi/epi.filter1.RData")
epi.filter1 <- subset_cells 
...
all.immune_barcode <- subset(scRNA,idents=c("Epithelial cell","Fibroblast",  "Endothelial cell"),
                                           invert = T)%>%colnames
barcodes <- c(epi.c8_barcode,all.immune_barcode) 
...
seurat.Control <- subset(scRNA, subset=group=="HCC") 
seurat.CRS <- subset(scRNA, subset=group=="HCC+CRS") #LS
...
######---------- 3. Visulization--------#############
pdf("netVisual_number_strength.pdf",width = 8,height =5)
par(mfrow = c(1,2))
...
##----------------------------------------------------------
## Epi.c8 interact with macro subclusters
##----------------------------------------------------------
setwd("~Workspace/06_mdd.liver/paper/clustering-macro/figures/cellchat")
setwd("./epi.c8-macro.allsub")

# load data
total_path <- "~Workspace/06_mdd.liver/paper/clustering-total/mdd.liver.filter1/"
epi_path <- "~Workspace/06_mdd.liver/paper/clustering-epi/"
macro_path <- "~Workspace/06_mdd.liver/paper/clustering-macro/"
# load mdd.filter1
...
seurat.Control <- subset(scRNA, subset=group=="HCC") 
seurat.CRS <- subset(scRNA, subset=group=="HCC+CRS") 

cellchat.Control <- createCellChat(seurat.Control@assays$RNA@data, meta = seurat.Control@meta.data, group.by = "celltype")
cellchat.CRS <- createCellChat(seurat.CRS@assays$RNA@data, meta = seurat.CRS@meta.data, group.by = "celltype")

# 2.2 Control(HCC) cellchat
cellchat <- cellchat.Control
...
# Plot
gg1 <- netAnalysis_signalingChanges_scatter(cellchat, idents.use = "Epi.c8")
ggsave("Fig6c-scatterplot-signaling changes of Epi.c8.pdf", gg1, width = 4, height =5)


pdf("Fig6d.COLLAGEN_chord.pdf", width = 12, height = 6)
...
p <- netVisual_bubble(cellchat, 
                      ...
                      remove.isolate = F)
ggsave("Fig6e-COLLAGEN-LR_bubble-Increased.pdf", p, width = 4, height =5)
