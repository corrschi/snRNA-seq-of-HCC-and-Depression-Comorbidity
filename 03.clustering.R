...
ggsave("sFig4c.MDD.liver-UMAP-byGroup.png", p, width = 5, height = 4)
Idents(MERGE) <- 'sample'
p <- DimPlot(MERGE,pt.size = 0.05,label=F,label.size=4,cols = color_sample[2:10],raster=F)
ggsave("sFig4d.MDD.liver-UMAP-bySample.png", p, width = 5, height = 4)
...
ggsave("Extended Fig5a.mdd.filter2-Dotplot-Anno-总体分群主Marker.png", p, width = 10, height = 4)
ggsave("Extended Fig5a.mdd.filter2-Dotplot-Anno-总体分群主Marker.pdf", p, width = 10, height = 4)
...
ggsave("sFig4e.mdd.filter2-Featureplot-Anno-总体分群主Marker-附图-有边框.png", p, width = 20, height = 15)
...
save(MERGE.markers,file = "~/ouyang/06_mdd.liver/paper/clustering-total/total_clustering.markers-ANNO.RData")
...
ggsave("Extended Fig5b.tissue_celltype_piechart.png", p, width = 9, height = 6)
write.csv(dat_plot,file='Extended Fig5b.Source_data_tissue_celltype_piechart.csv')
...
for (i in 1:n_pages) {
  ...
  png(paste0("Extened Fig5c.Vln_Chr_Page", i, ".png"), width = 10, height = 8, res = 300, units = "in")
  print(VlnPlot(MERGE, features = gene_subset, pt.size = 0.001, ncol = 2))
  dev.off()
}

png(paste0("Extened Fig5c.Vln_Chr_Page1 in all.png"), width = 24, height = 12, res = 400, units = "in")
VlnPlot(MERGE,                          
