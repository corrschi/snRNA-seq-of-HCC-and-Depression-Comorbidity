...
setwd("~/Workspace/06_mdd.liver/paper/clustering-macro")
## load mdd.filter1
...
save(MERGE.markers,file = "~/Workspace/06_mdd.liver/paper/clustering-total/total_clustering.markers.RData")
...
MERGE <- RenameIdents(object = MERGE, 
                      ...
                      `Macro_Spp1` = "Macro_c2",
                      `Mono_Ly6c2` = "Mono_c1",
                      `Macro_Cxcl1` = "Macro_c3",
                      `Macro_Cxcl9` = "Macro_c4",
                      `Kuppfle_Clec4f` = "Kuppfle_c1", 
                      `Macro_Stmn1` = "Macro_c5",
                      `Macro_Pdpk1` = "Macro_c6",
                      `Mono_S100a9` = "Mono_c2",
                      `Macro_Cxcl13` = "Macro_c7")
MERGE[["Anno.macro"]] <- Idents(object = MERGE)

MERGE$Anno.macro <- factor(MERGE$Anno.macro, 
                           levels = c("Mono_c1","Mono_c2", "Macro_c1", "Macro_c2", 
                                      "Macro_c3", "Macro_c4", 
                                      "Macro_c5", "Macro_c6",  "Macro_c7",
                                      "Kuppfle_c1"))
Idents(MERGE) <- "Anno.macro"
...
ggsave("Fig6b.Dotplot-Anno-macro subcluster Marker.png", p, width = 8, height = 4)
...
library(viridis)
dat_plot <- MERGE
cluster.average <- AverageExpression(dat_plot)[[1]] 


genes.select <- c("Cxcl2", "Cxcl1","Ccl3","Cxcl3","Ccl4",  "Ccl9", "Ccl2", "Ccl6","Ccl5", 
                  "Il1a","Il1b","Il6","Tnf") # rank from marker gene
...
pdf("Supplementary Figure 14b-Heatmap of Chemokines and Inflammatory Cytokines.pdf", width = 9, height = 6)
