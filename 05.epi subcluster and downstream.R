...
setwd("~/Workspace/06_mdd.liver/paper/clustering-epi")
...
## group name change
epi.filter1$group <- as.character(epi.filter1$group)  
epi.filter1$group[epi.filter1$group == "Stress"] <- "HCC+CRS"
epi.filter1$group <- factor(epi.filter1$group, levels = c("HCC", "HCC+CRS")) 


#Plots
# Fig5c
png(paste0("Fig5c-",object.name,"_res", res.use ,"_dim", dim.use ,"_all_by_harmony_dimplot.png"), width = 7, height = 5, res = 400, units = "in")
...
ggsave("FeaturePlote-epi.Marker.png", plot = p, width = 12, height = 3, units = "in", dpi = 400)



library(Seurat)
library(Nebulosa)
png("Fig5d.density plot-Chrna9.png",  width = 5, height = 4, res = 400, units = "in")
...
png("irGSEA.ridgeplot-KEGG-NEUROACTIVE-LIGAND-RECEPTOR-INTERACTION.png", 
...
png("Fig6f-Epi.c8-Vln-Chrna9.png", width = 3, height = 3, res = 400, units = "in")
p <- VlnPlot(dat_plot,
             features = c("Chrna9"),
             pt.size = 0.001,
             ncol = 1)
...
# load data
setwd("~/Workspace/06_mdd.liver/paper/clustering-epi/ClusterGVis")
library(ClusterGVis)
library(Seurat)
pbmc <-epi
## data check
...
p<- visCluster(object = st.data,
               plot.type = "line")
ggsave(paste0( "sFig12e.Line plot.png"),p,width = 12,height = 9)
