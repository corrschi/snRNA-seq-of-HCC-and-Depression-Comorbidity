...
setwd("～/Workspace/ouyang/06_mdd.liver/paper/doublet-new/qc-plots-chi")
library(Seurat)
library(dplyr)
library(ggplot2)

sample_list <- c( "Stress_1", "Stress_2", "Stress_3","Control_1", "Control_2", "Control_3")
base_path <- "～/Workspace/ouyang/06_mdd.liver/singlesample"
...
pdf("sFig4b-QC_density_nFeature.pdf", width = 4, height = 5)
ggplot(qc_meta_all, aes(x = nFeature_RNA)) +
  geom_density(fill = "lightgreen", color = "black") +
...
windowsFonts(Times = windowsFont("Times New Roman"))  # 注册字体
pdf("sFig4b-QC_density_percentMT.pdf", width = 4, height = 5)
ggplot(qc_meta_all, aes(x = percent.mt)) +
  geom_density(fill = "lightgreen", color = "black") +
