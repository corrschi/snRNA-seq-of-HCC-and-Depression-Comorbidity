...
library(EPIC)
library(dplyr)
setwd("~/Workspace/06_mdd.liver/paper/tcga")
...
pairs <- lapply(
  seq_len(nrow(expand.grid(ligand_genes, receptor_genes))),
  function(i) as.character(expand.grid(ligand_genes, receptor_genes)[i, ]))
...
library(broom)
library(ggplot2)
merged_data$group <- relevel(factor(merged_data$group), ref = "Low") 
...
ggplot(df, aes(x = estimate, y = term)) +
  ...
  geom_text(aes(x = 3.5, label = label), hjust = 0, size = 4.0) +  
